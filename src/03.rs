...
fn into_hash_set<'a, T: Eq + Hash + Copy, S: BuildHasher>(
    isect: Intersection<'a, T, S>,
) -> HashSet<T> {
...
fn common_elements(strs: &[String]) -> HashSet<char> {
    return strs
        .iter()
        .map(|l| item_types(l))
        .reduce(|acc, item| into_hash_set(acc.intersection(&item)))
        .unwrap();
}
